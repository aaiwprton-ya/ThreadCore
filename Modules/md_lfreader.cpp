...
//    : Thread(iface)
//{
//    _add_pack_link_(0, std::string(P_NAME[0]), std::string("IOSystem"));
//    _add_pack_link_(1, std::string(P_NAME[1]), std::string(M_NAME));
//}
...
//    PK_FrameBuffer *p_frame_buffer = static_cast<PK_FrameBuffer*>(packs[0]);
//    while (true)
//    {
...
//    {
//        pk_keys.insert(PK_KEY(P_NAME[0], new PK_FrameBuffer::Key()));
...
//    for (size_t i = 0; i < P_NAME.size(); ++i)
//    {
//        (*packs)[*static_cast<PK_FrameBuffer::Key*>(pk_keys.at(P_NAME[i]))]
//                = PK_FrameBuffer(P_NAME[i], M_NAME, std::this_thread::get_id());
//    }
//}
